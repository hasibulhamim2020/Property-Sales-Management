...
(select count(c.fid) from `tbl_flat_info` c where a.proj_code=c.proj_code and (status!='booked' or status is NULL)) as unsold, 
...
$allotment_no=$fid; $flat_show=',c.flat_no as Plot_no'; $flat_con=' and c.proj_code='.$proj_code.' and c.fid=\''.$fid.'\' ';}
		$sql="select a.rec_date as tr_date,a.cheq_no,a.flat_no as Plot_no,b.proj_name".$flat_show.",a.rec_amount as total_amt from tbl_receipt a,tbl_project_info b,tbl_flat_info c where c.fid=a.fid and c.proj_code=b.proj_code ".$proj_con.$date_con.$flat_con." order by a.rec_date";
		break;
		case 7:
...
{$project_name=find_a_field('tbl_project_info','proj_name','proj_code='.$proj_code); 
$allotment_no=$fid; $flat_show=',a.fid as allot_no'; $flat_con=' and d.proj_code='.$proj_code.' and a.fid=\''.$fid.'\' ';}
		if(isset($t_date)) 
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.rec_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		$sql="SELECT a.`rec_date`,a.`rec_no` as m_r_no,b.proj_name,a.fid,c.party_name,a.`rec_amount`,
...
WHERE b.proj_code=d.proj_code and d.fid=a.fid and a.party_code=c.party_code ".$proj_con.$date_con.$flat_con.$mr_con;
		break;
...
{$project_name=find_a_field('tbl_project_info','proj_name','proj_code='.$proj_code); 
$allotment_no=$fid; $flat_show=',a.fid as allot_no'; echo $flat_con=' and d.proj_code='.$proj_code.' and d.fid=\''.$fid.'\' ';}
		
		if(isset($t_date)) 
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.inst_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		echo $sql="select c.proj_name as project_name,a.flat_no as Plot_no,b.party_name as client_name,a.inst_date,a.inst_amount as payable_amt,a.rcv_amount as received_amt 
		...
		and a.fid=d.fid and rcv_status=0 ".$proj_con.$date_con.$flat_con.$party_con." order by a.inst_date";
		break;
	case 12:
...
$allotment_no=$fid; $flat_show=',a.fid as allot_no'; $flat_con=' and a.proj_code='.$proj_code.' and a.fid=\''.$fid.'\' ';}
		...
		if(isset($t_date)) 
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.inst_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		$sql="select c.proj_name as project_name,a.fid as allot_no,b.party_name as client_name,a.inst_date,a.inst_amount as payable_amt,a.rcv_amount as received_amt 
		
		from tbl_flat_cost_installment a, tbl_party_info b, tbl_project_info c,tbl_flat_info d 
		
		where a.proj_code=c.proj_code and d.party_code=b.party_code and a.proj_code=d.proj_code and a.fid=d.fid ".$proj_con.$date_con.$flat_con.$party_con." order by a.inst_date";
		break;
...
$allotment_no=$fid; $flat_show=',a.flat_no as allot_no'; $flat_con=' and d.proj_code='.$proj_code.' and d.fid=\''.$fid.'\' ';}
		...
		if(isset($t_date)) 
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.inst_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		 $sql="SELECT e.pay_desc,a.inst_no, c.proj_name AS project_name,a.fid AS Plot_no,  a.inst_date, a.inst_amount AS payable_amt, a.rcv_date AS receive_date, a.rcv_amount AS receive_amt
FROM 
...
$allotment_no=$fid; $flat_show=',c.flat_no as allot_no'; $flat_con=' and c.proj_code='.$proj_code.' and c.fid=\''.$fid.'\' ';}
		if(isset($t_date)) 
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.rec_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		echo $sql="select a.rec_date as tr_date,b.proj_name".$flat_show.",a.rec_amount as total_amt from tbl_receipt a,tbl_project_info b,tbl_flat_info c where c.fid=a.fid and a.pay_mode=0 and c.proj_code=b.proj_code ".$proj_con.$date_con.$flat_con." order by a.rec_date";
		break;
	case 25:
...
$allotment_no=$fid; $flat_show=',c.flat_no as allot_no'; $flat_con=' and c.proj_code='.$proj_code.' and c.fid=\''.$fid.'\' ';}
		if(isset($t_date)) 
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.rec_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
	echo $sql="select a.rec_date as tr_date,a.cheq_no,b.proj_name".$flat_show.",a.rec_amount as total_amt from tbl_receipt a,tbl_project_info b,tbl_flat_info c where c.fid=a.fid and a.pay_mode=1 and c.proj_code=b.proj_code ".$proj_con.$date_con.$flat_con." order by a.rec_date";
		break;
...
{$client_name=find_a_field('tbl_party_info','party_name','party_code='.$party_code); $party_con=' and c.party_code='.$party_code;}
		...
		if(isset($t_date)) 
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.booked_on between \''.$fr_date.'\' and \''.$to_date.'\'';}
	echo $sql="SELECT b.proj_name,a.flat_no as Plot_no,a.flat_size as Plot_size,i.sr_executive_commission,i.team_leader_commission,i.group_leader_commission,i.other_commission,
