...
if($_POST['section_name']!=''){$section_con = ' and s.section_id="'.$_POST['section_name'].'"';};
 ...
 $sql="SELECT c.party_code as customer_id, c.party_name as Customer_name, b.proj_name as project_name,(select section_name from add_section where section_id=a.section_name) as section_name, road_no, a.flat_no as Plot_No, a.flat_size as Plot_Size, a.booked_on, a.total_price,a.sr_status as status FROM `tbl_flat_info` a,tbl_project_info b,tbl_party_info c, add_section s WHERE a.section_name=s.section_id and a.proj_code=b.proj_code and a.party_code=c.party_code ".$proj_con.$date_con.$flat_con.$party_con.$section_con.$flat_con;
...
if($_POST['section_name']!=''){$section_con = ' and a.section_name="'.$_POST['section_name'].'"';};
 ...
 $sql="select  d.party_name as customer_name ,d.party_code, c.proj_name,b.section_name,e.road_no,a.flat_no as plot_name,sum(a.inst_amount) as total_payable,sum(a.rcv_amount) as total_received, concat((sum(a.rcv_amount)/((sum(a.inst_amount)/100)*1)),'%') as Percentage  from tbl_flat_cost_installment a, add_section b, tbl_project_info c, tbl_party_info d, tbl_flat_info e where  1 and e.sr_status='AGREEMENT'  and a.fid=e.fid   and a.section_name=b.section_id and a.proj_code=c.proj_code and a.party_code=d.party_code  ".$proj_con.$date_con.$flat_con.$party_con.$section_con.$flat_con." group by a.fid ";
...
$sql="SELECT b.proj_name,(select section_name from add_section where section_id=a.section_name) as section_name,road_no,a.flat_no as Plot_No,a.flat_size as Plot_Size,a.total_price,c.party_code as customer_id,c.party_name as Customer_name FROM `tbl_flat_info` a,tbl_project_info b,tbl_party_info c, add_section s WHERE a.sr_status='SOLD' and a.section_name=s.section_id and a.proj_code=b.proj_code and a.party_code=c.party_code ".$proj_con.$date_con.$flat_con.$party_con.$section_con.$flat_con;
...
$sl = 0;
while($row = mysql_fetch_object($qq)){
...
if(5000000 <= $row->total_amt && $row->total_amt <= 10000000){ echo $row->total_amt ;}
...
if(10000000 <= $row->total_amt && $row->total_amt <= 15000000){ echo $row->total_amt ;}
...
if(15000000 <= $row->total_amt ){ echo $row->total_amt ;}
...
echo '<tr><td>'.$s++.'</td><td>'.$r->fid.'</td><td>'.$r->rec_date.'</td><td><a href="../../common/voucher_print_report.php?rec_no='.$r->m_r_no.'" target="_blank">'.$r->m_r_no.'</a></td><td>'.$r->voucher_no.'</td><td>'.$r->party_name.'</td><td>'.$r->proj_name.'</td><td>'.$r->road_no.'</td><td>'.$r->plot_no.'</td><td>'.$r->section_name.'</td><td>'.number_format($r->rec_amount).'</td><td>'.$b_type.'</td></tr>';
...
<tr><td colspan="9"></td><td align="right"><b>Total : </b></td><td><b>'.number_format($tot_rec_amt).'</b></td><td></td></tr>
...
<tr><th>S/L</th><th>Party Code</th><th>Party Name</th><th>Project Name</th><th>Section Name</th><th>Road No</th><th>Plot NO</th><th>Plot Size</th><th>Total Price</th><th>Received Amount</th><th>Pending Amount</th></tr>';




$sql1 = "select a.fid from tbl_receipt a, tbl_flat_info b where 1 and a.fid=b.fid ".$proj_con." group by a.fid order by a.fid";
$sl = 0;
...
$rr=mysql_fetch_object(mysql_query($se));
echo $rr->party_name;
...
$rrrr=mysql_fetch_object(mysql_query($seee));
...
$rrr=mysql_fetch_object(mysql_query($see));
echo $rrr->tot_amt;
...
echo '</td><td>'.$pending_amount =($rrr->tot_amt-$rr->rec_Amount) .'</td></tr>';
...
echo '<tr style="font-weight: bold;"><td colspan="7">Total :</td><td>'.$tot_f_size.'</td><td>'.$tot_amtt.'</td><td>'.$tot_rec_amt.'</td><td>'.$tot_p_amt.'</td></tr>';
...
if($_POST['section_name']!=''){$section_con = ' and s.section_id="'.$_POST['section_name'].'"';};
	...
	$sql="select b.party_name as client_name, c.proj_name as project_name, (select section_name from add_section where section_id=a.section_name) as section_name, road_no, a.flat_no as Plot_no, a.inst_date as installment_date,a.inst_amount as payable_amt, a.rcv_amount as received_amt,a.`rec_no` as m_r_no 
...
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.rec_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
	 ...
	 $sql="select d.party_name".$flat_show.", a.rec_date as tr_date,a.cheq_no,a.cheq_date,b.proj_name, c.flat_no as plot_no, c.road_no, sum(a.rec_amount) as total_amt from tbl_receipt a,tbl_project_info b,tbl_flat_info c,tbl_party_info d where a.party_code=d.party_code and c.fid=a.fid and a.pay_mode=3 and c.proj_code=b.proj_code ".$proj_con.$date_con.$flat_con." group by a.cheq_no order by a.rec_date";
...
/*$sql="SELECT b.proj_name,a.flat_no as Plot_no,concat(' &emsp;',a.flat_size) as Plot_size,(select section_name from add_section where section_id=a.section_name) as section_name,i.sr_executive_commission,i.team_leader_commission,i.group_leader_commission,i.other_commission,
	...
	a.total_price,(select PBI_NAME from personnel_basic_info where PBI_ID=c.sr_executive) as sr_executive_name,(select PBI_NAME from personnel_basic_info where PBI_ID=c.team_leader) as team_leader,(select PBI_NAME from personnel_basic_info where PBI_ID=c.group_leader) as group_leader, c.party_name FROM `tbl_flat_info` a,tbl_project_info b,tbl_party_info c,tbl_flat_cost_installment i WHERE (i.sr_executive_commission+i.team_leader_commission+i.group_leader_commission+i.other_commission)!='' and a.party_code=i.party_code and a.proj_code=b.proj_code and a.party_code=c.party_code ".$proj_con.$date_con.$flat_con.$party_con." group by i.id";*/
	 ...
	 $sql="SELECT c.party_name, b.proj_name as project_name, a.flat_no as Plot_no, concat(' &emsp;',a.flat_size) as Plot_size,(select section_name from add_section where section_id=a.section_name) as section_name,
	 
	 a.sr_status as status, 
	 
	 a.total_price,
	 
	 sum(i.rcv_amount) as total_received_amount,
...
/* 	 $sql="SELECT b.proj_name,(select section_name from add_section where section_id=a.section_name) as section_name,a.flat_no as Plot_no,a.flat_size as Plot_size,(select PBI_NAME from personnel_basic_info where PBI_ID=c.sr_executive) as sr_executive_name,i.sr_executive_commission,i.rcv_amount as receive_amount,a.total_price, a.status,a.booked_on,c.party_name FROM `tbl_flat_info` a,tbl_project_info b,tbl_party_info c,tbl_flat_cost_installment i WHERE i.sr_executive_commission!='' and  a.party_code=i.party_code and a.proj_code=b.proj_code and a.party_code=c.party_code ".$emp_con.$proj_con.$date_con.$flat_con.$party_con." group by i.id"; */
	 ...
	 $sql="SELECT c.party_name, b.proj_name as project_name,(select section_name from add_section where section_id=a.section_name) as section_name, a.flat_no as Plot_no, a.flat_size as Plot_size, a.sr_status as status, a.total_price, i.rcv_amount as received_amount, i.sr_executive_commission, (select PBI_NAME from personnel_basic_info where PBI_ID=c.sr_executive) as sr_executive_name  FROM `tbl_flat_info` a,tbl_project_info b,tbl_party_info c,tbl_flat_cost_installment i WHERE i.sr_executive_commission!='' and  a.party_code=i.party_code and a.proj_code=b.proj_code and a.party_code=c.party_code ".$emp_con.$proj_con.$date_con.$flat_con.$party_con." group by i.id";
...
/*$sql="SELECT b.proj_name,(select section_name from add_section where section_id=a.section_name) as section_name,a.flat_no as Plot_no,a.flat_size as Plot_size,(select PBI_NAME from personnel_basic_info where PBI_ID=c.group_leader) as group_leader,i.group_leader_commission,i.other_commission,i.rcv_amount as receive_amount, a.total_price, a.status,a.booked_on,c.party_name FROM `tbl_flat_info` a,tbl_project_info b,tbl_party_info c,tbl_flat_cost_installment i WHERE  i.group_leader_commission and a.party_code=i.party_code and a.proj_code=b.proj_code and a.party_code=c.party_code ".$emp_con.$proj_con.$date_con.$flat_con.$party_con." group by i.id";*/
	 ...
	 $sql="SELECT c.party_name, b.proj_name as project_name,(select section_name from add_section where section_id=a.section_name) as section_name,a.flat_no as Plot_no,a.flat_size as Plot_size, a.sr_status as status, a.total_price, i.rcv_amount as received_amount, i.group_leader_commission, (select PBI_NAME from personnel_basic_info where PBI_ID=c.group_leader) as group_leader
	  ...
	  FROM `tbl_flat_info` a,tbl_project_info b,tbl_party_info c,tbl_flat_cost_installment i WHERE  i.group_leader_commission and a.party_code=i.party_code and a.proj_code=b.proj_code and a.party_code=c.party_code ".$emp_con.$proj_con.$date_con.$flat_con.$party_con." group by i.id";
