...
echo $sql="select a.pay_code,a.inst_no,a.inst_amount,a.rec_amount,b.proj_code,b.build_code,b.flat_no from tbl_receipt_details a,tbl_receipt b where a.rec_no=".$del_rec_no." and a.rec_no=b.rec_no";
...
$query=db_query($sql);
	if(mysqli_num_rows($query)>0)
	{
		...
		$data_old=mysqli_fetch_object($query_old);
		else $data_old=NULL;
			if($data_old)
...
db_query($update_tbl_flat_cost_installment);
		}
	}
...
db_query($delete_tbl_receipt);
db_query($delete_tbl_receipt_detail);
}
?>
<style type="text/css">
<!--
...
?>
				...
				<input name="payment" type="button" class="btn" id="payment" value="Payment" onclick="getData('search_client1.php', 'nid', document.getElementById('flat').value);" /></div>
				</fieldset>		  </td>
