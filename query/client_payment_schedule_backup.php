...
$page  = 'client_payment_schedule.php';
...
// If form submitted, fetch client details and payment schedule
if (isset($_POST['submit']) && $flat > 0) {
    // Fetch flat details and party info
    ...
    if (db_num_rows($res_flat) > 0) {
        ...
        // Get total instalment and received amounts
                            ...
                            <span id="flat_span">
                                <select name="flat" id="flat" class="form-control">
                                    ...
                                    if ($proj_code && $section_name) {
                                        // Load flats via PHP if needed (initial load)
    ...
    <div class="row mt-3">
        <div class="col-12">
            <div class="container n-form1">
                <h4 class="n-form-titel1">Payment Schedule</h4>
                <?php if ($flat > 0) { ?>
                    <?php
                    // Fetch instalment details
                    $sql_inst = "SELECT a.rec_no, c.pay_desc, a.inst_no, a.inst_date, a.inst_amount, a.rcv_amount
                    ...
                    <div class="tabledesign2">
                        <table id="table_head" class="table table-bordered" cellspacing="0">
                            ...
                            <tbody>
                                <?php
                                $i = 0;
                                while ($row = db_fetch_row($res_inst)) {
                                    $i++;
                                    $cls = ($i % 2 == 0) ? ' class="alt"' : '';
                                ?>
                                <tr<?= $cls ?>>
                ...
                <?php } else { ?>
                    <!-- Placeholder table when no flat selected -->
                    <div class="tabledesign2">
                        <table id="table_head" class="table table-bordered" cellspacing="0">
                            <thead>
                                <tr class="bgc-info">
                            ...
                            <tbody>
                                <tr><td colspan="5" class="text-center">Please select a project, section, and flat to view schedule.</td></tr>
                            </tbody>
                        </table>
                    </div>
                <?php } ?>
